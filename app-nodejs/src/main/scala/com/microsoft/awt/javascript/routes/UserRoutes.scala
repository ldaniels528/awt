...
import com.microsoft.awt.javascript.data.UserDAO._
import com.microsoft.awt.javascript.data.UserData._
import com.microsoft.awt.javascript.data.{UserDAO, UserData}
import com.microsoft.awt.javascript.forms.ProfileEditForm
import com.microsoft.awt.javascript.models.User
...
object UserRoutes {
  ...
  def init(app: Application, dbFuture: Future[Db])(implicit ec: ExecutionContext, mongo: MongoDB, require: NodeRequire) = {
    ...
    implicit val client = RequestClient()
    ...
    app.get("/api/users", (request: Request, response: Response, next: NextFunction) => getUsers(request, response, next))
  ...
  def getUserByEmail(request: Request, response: Response, next: NextFunction)(implicit ec: ExecutionContext, mongo: MongoDB, userDAO: Future[UserDAO]) = {
    request.query.get("primaryEmail") match {
      case Some(primaryEmail) =>
        userDAO.flatMap(_.findByEmail(primaryEmail)) onComplete {
          case Success(Some(user)) => response.send(user); next()
  ...
  def getUserByUsername(request: Request, response: Response, next: NextFunction)(implicit ec: ExecutionContext, mongo: MongoDB, userDAO: Future[UserDAO]) = {
    request.query.get("username") match {
      case Some(username) =>
        userDAO.flatMap(_.findByUsername(username)) onComplete {
          case Success(Some(user)) => response.send(user); next()
  ...
  def getUsers(request: Request, response: Response, next: NextFunction)(implicit ec: ExecutionContext, mongo: MongoDB, userDAO: Future[UserDAO]) = {
    val ids = request.query.get("ids").map(list => js.Array(list.split("[ ]").map(_.trim): _*)) getOrElse emptyArray
    val query = if (ids.nonEmpty) doc("_id" $in ids.map(_.$oid)) else doc()
    userDAO.flatMap(_.find(query).toArrayFuture[User]) onComplete {
  ...
  /**
    * Update a user by ID
    * @example PUT /api/user/5633c756d9d5baa77a714803
    */
  def updateUser(request: Request, response: Response, next: NextFunction)(implicit ec: ExecutionContext, mongo: MongoDB, userDAO: Future[UserDAO]) = {
    val userID = request.params("userID")
    val form = request.bodyAs[ProfileEditForm]
    userDAO.flatMap(_.updateOne(filter = doc("_id" -> userID.$oid), update = $set(doc(
      "avatarURL" -> form.avatarURL, "firstName" -> form.firstName, "lastName" -> form.lastName,
      "primaryEmail" -> form.primaryEmail, "title" -> form.title
    ))).toFuture) onComplete {
  ...
  def getFollowers(request: Request, response: Response, next: NextFunction)(implicit ec: ExecutionContext, mongo: MongoDB, userDAO: Future[UserDAO]) = {
    ...
    userDAO.flatMap(_.findFollowers(followeeID)) onComplete {
      case Success(followers) => response.send(followers); next()
      case Failure(e) => response.internalServerError(e); next()
    }
  }
